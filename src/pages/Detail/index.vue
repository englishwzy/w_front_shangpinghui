          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  changepirce="0"
                  :class="{ active: s.isChecked == 1 }"
                  v-for="s in spuSaleAttr.spuSaleAttrValueList"
                  :key="s.id"
                  @click="checkSaleAttr(spuSaleAttr.spuSaleAttrValueList, s)"
                >
                  {{ s.saleAttrValueName }}
                </dd>
              </dl>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="goCard">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("detailAbout/getGoodDetail", this.$route.params.skuid);
  },
  methods: {
    // 排他
    checkSaleAttr(bigs, s) {
      bigs.forEach((item) => {
        item.isChecked = "0";
      });
      s.isChecked = "1";
    ...
    async goCard() {
      ...
      try {
        const result = await this.$store.dispatch("detailAbout/addCart", {
