  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="ad in address"
        :key="ad.id"
        @click="changeDefaultAddress(ad, address)"
      >
        <span class="username" :class="{ selected: ad.isDefault == '1' }">{{
          ad.consignee
        }}</span>
        <p>
          <span class="s1">{{ ad.userAddress }}</span>
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="li in orderInfo.detailArrayList"
          :key="li.skuId"
        >
          <li>
            <img :src="li.imgUrl" alt="" style="width: 200px; height: 200px" />
          </li>
          <li>
            <p style="width: 200px">
              {{ li.skuName }}
    ...
    <div class="sub clearFix">
      <A class="subBtn" @click="handOrder">提交订单</A>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("tradeAbout/getUserGoods");
  },
  computed: {
    ...mapState("tradeAbout", ["address", "orderInfo"]),
    defaultAddress() {
      console.log(this.address.find((item) => item.isDefault == "1"));
      return this.address.find((item) => item.isDefault == "1") || {};
    },
  },
  methods: {
    changeDefaultAddress(ad, address) {
